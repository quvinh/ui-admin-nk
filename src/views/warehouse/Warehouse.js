...
const Warehouse = () => {
    ...
    const handleAddWarehouse = () => {
        ...
        console.log(data);
        Promise.all([postData('/api/admin/warehouse/store?token=' + getToken(), data)])
            .then(res => {
    ...
    const handleUpdate = (e) => {
        ...
        Promise.all([putData('/api/admin/warehouse/update/' + id + '?token=' + getToken(), warehouse)])
            .then(response => {
                // console.log(data)
    ...
    const handleUpdateStatusWarehouse = (e) => {
        ...
        console.log(id)
        console.log(warehouse)
        Promise.all([putData('/api/admin/warehouse/statusWarehouse/' + id + '?token=' + getToken(), warehouse)])
            .then(response => {
                // console.log(data)
                console.log('Edited successfully ^^')
                setNullWarehouse()
    ...
    const handleReload = () => {
        Promise.all([getData('/api/admin/warehouse?token=' + getToken())])
            .then(function (res) {
    ...
    const handleCount = (id) => {
        Promise.all([getData('/api/admin/warehouse/amountShelf/' + id + '?token=' + getToken()),])
            .then(function (res) {
                setCount(res[0].data)
            })
    }
    const handleDelete = (e, id) => {
        // console.log(id)
        // if (count === 0) {
        Promise.all([delData('/api/admin/warehouse/delete/' + id + '?token=' + getToken())])
    ...
    useEffect(() => {
        Promise.all([
            getData(getRoleNames() === 'admin' ? (
                '/api/admin/warehouse?token=' + getToken()
            ) : getDataWarehouseID().length > 0 && (
                '/api/admin/warehouse/'+ getUserID() +'?token=' + getToken()
            )),

            // getData('/api/admin/warehouse/warehouseShow?token=' + getToken()),
